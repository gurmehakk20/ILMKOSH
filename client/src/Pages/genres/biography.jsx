...
const Romance = () => {
    ...
    const fetchBooks = async () => {
      try {
        const response = await axios.get(
          `https://www.googleapis.com/books/v1/volumes?q=biography&key=AIzaSyCSvSsw52clyt8ozO8HuXn6u6H7_PwtfOU&maxResults=39`
        );
  ...
  const handleAddToCart = () => {
    navigate('/mybooks', { state: { book: selectedBook } }); // Redirect with book data
  };
      ...
      {selectedBook && (
        <div className="popup-container">
          <div className="popup-content">
            ...
            <button onClick={handleAddToCart}>Add to Your Books</button>
          </div>
        </div>
      )}
    </div>
